<?php
    ...
    if(!is_dir('USERFILES')){
        ...
        echo 'alert("By hosting WOSP or otherwise using its code or resources, you are agreeing to the End User License Agreement which will open in a new window/tab when you click anywhere on the aOS desktop.\nWOSP is provided FREE OF CHARGE. If you paid money for this software, please contact mineandcraft12@gmail.com");window.tosClick=function(){window.open("eula.txt","_blank");window.removeEventListener("click",window.tosClick)};window.addEventListener("click",window.tosClick);';
    }
            ...
            if(file_exists('USERFILES/'.$changeKey.'/aOSpassword.txt')){
                ...
                if(strlen($targetPass) === 64){
                    ...
                    if(password_verify($changePass, $targetPass)){
                        setcookie('keyword', $changeKey, time() - 86400, '/WOSP');
                        setcookie('keyword', $changeKey, time() - 86400, '/WOSP/aosBeta.php');
                        // tell the browser the new user name
                        setcookie('keyword', $changeKey, time() + 321408000, "/");// time() + 321408000);
    ...
    if(!isset($_COOKIE['last_aOS_login'])){
        setcookie('keyword', $_COOKIE['keyword'], time() - 86400, '/WOSP');
        setcookie('keyword', $_COOKIE['keyword'], time() - 86400, '/WOSP/aosBeta.php');
